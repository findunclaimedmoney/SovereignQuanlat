...
## Next Tasks
1. USER ACTION (ready now): redeploy to production from Emergent dashboard — deployment health check PASSED 2026-08-23 post video/Proof changes; pushes new video + Real Engine Run Proof block + video preload/fallback tweak. 2. Rotate LICENCE_HMAC_SECRET before real sales (user deferred). 3. Upload logo in Stripe Dashboard → Settings → Branding (file at /app/sq-logo-stripe.png). 4. Optional: user uploads real equity_curve.csv from workstation logs to replace representative curve with true path. 5. Stripe KYC.
