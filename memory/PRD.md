...
## Implemented (2026-08-26, round 17)
- AUTOMATIC ED25519 SIGNING LIVE (user reversed manual-signing decision, chose merge): generate_licence_key now signs Ed25519 with LICENCE_PRIVATE_KEY (env LICENCE_ED25519_PRIVATE_KEY_B64). Private key verified to derive EXACTLY the public key baked into the shipped app (Cdg9OUuI9DgWIKmkmiAIKogaXe7qwfuojhXhtiHJhs8=). Full chain proven with real code+keys: server-minted Institutional key → app verify → valid, expires 2027-08-26. fulfil_order + upgrade flow mint instantly again (licence_status="issued"). KEPT from manual era: owner sale notification (notify_owner_sale: "NEW SALE — key auto-issued", includes issued key for records) and admin Issue Key box (manual override/resend — also services any gap orders created while production ran the awaiting_manual_issue build). manual_licence_pending template deleted; PaymentSuccess awaiting branch retained as dormant fallback. All copy swept back to instant delivery (concierge, memory core, knowledge entries, renewal email, guide page, narrations 1+5 re-cached).

## Next Tasks
1. USER ACTION: redeploy to production (pushes Ed25519 zip + auto-signing backend). 2. USER ACTION: laptop e2e — download zip, pip install -r requirements.txt, activate a test key. 3. Optional: real sale test then refund. 4. Upload logo in Stripe Dashboard → Settings → Branding. 5. Optional: real equity_curve.csv upload. 6. Stripe KYC.
