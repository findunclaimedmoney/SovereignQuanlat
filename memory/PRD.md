...
- GATED (waiting on user): app swap to Ed25519 app (app_v2.py at backend/new_workstation/, has expiry enforcement, placeholder PUBLIC_KEY_B64). User must run generate-keypair on their laptop and paste the PUBLIC_KEY_B64 line; then bake pubkey in, replace bundle/app.py + requirements.txt (adds requests/pyarrow/cryptography), rebuild zip, redeploy. Until swap: zip still serves rotated-HMAC app — do NOT launch sales before swap+redeploy (keys minted by user's Ed25519 tool won't activate the old app). Zip is built ONLY from bundle/ (app.py, README, requirements, run scripts) — seller_only/ physically cannot be included (confirmed to user 2026-08-26).
...
## Next Tasks
1. USER ACTION: run `python license_signing_tool.py generate-keypair` on laptop, paste PUBLIC_KEY_B64 line → agent completes app swap + zip rebuild. 2. USER ACTION: redeploy to production (sovereignquant.com.au) after swap. 3. Upload logo in Stripe Dashboard → Settings → Branding (file at /app/sq-logo-stripe.png). 4. Optional: user uploads real equity_curve.csv to replace representative curve. 5. Stripe KYC.
